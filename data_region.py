...
class PVTable(MultiListbox):
    ...
    def __init__(self, parent, conf):
        MultiListbox.__init__(self, parent, ("No Data\n",))
    ...
    def update_ox(self, conf):
        ...
        for ox in conf.open_experiments:
            ...
            # HEADER
            headers.extend(self.get_headers(ox))

            # DATA(s)

        self.update_table(headers, self.get_data(ox))

    @classmethod
    def get_headers(cls, ox):
        # Add Description (Table Header)
        for i in range(ox.get_nvalues()):
            yield "%s\n(%s)" % (ox.get_desc(i), ox.get_units(i))

    def get_data(self, ox):

        data = []

        for row in ox.sqlvalues:
            data.append(row)
    ...
    def update_table(self, headers, data):
        ...
        # this is quiet hackish to work...
        MultiListbox.__init__(self, self.parent, headers)
